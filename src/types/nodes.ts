...
export interface StoryRootNodeData {
  ...
  image?: string;
  /**
   * Background music for the story root node.
   * Stored as a base64 data URL or blob URL for persistence.
   * Can be assigned from the RPG music library or uploaded manually.
   */
  backgroundMusic?: string;
  /** TTS narration audio generated by Photo Story player (blob URL or data URL) */
  voiceoverAudio?: string;
  /**
   * Dictionary of entity state changes during this segment.
   * Key = entity ID (e.g. "entity_abc123").
   * Value = freeform description of ALL changes for that entity during this node's time interval:
   * physical, emotional, psychological, social, medical, belief, goal, intention changes.
   * Each entry also includes the rationale (why/how the change happened).
   */
...
export interface PlotNodeData {
  ...
  image?: string;
  /**
   * Background music for the plot node.
   * Stored as a base64 data URL or blob URL for persistence.
   * Can be assigned from the RPG music library or uploaded manually.
   */
  backgroundMusic?: string;
...
export interface ActNodeData {
  ...
  turningPoint?: string;
  /**
   * Background music for the act node.
   * Stored as a base64 data URL or blob URL for persistence.
   * Can be assigned from the RPG music library or uploaded manually.
   */
  backgroundMusic?: string;
  ...
  voiceoverAudio?: string;
  /**
   * If true, this "act" node represents an episode (TV series / web serial structure)
   * rather than a traditional screenplay/novel act. Episode nodes use teal theming
   * and labels like "Episode N" / "Cliffhanger" instead of "Act N" / "Turning Point".
   * Optional — defaults to false for backwards compatibility.
   */
  isEpisode?: boolean;
  /**
   * Dictionary of entity state changes during this segment.
...
export interface CoWriteSceneData {
  ...
  image?: string;
  /**
   * Background music for the co-write scene node.
   * Stored as a base64 data URL or blob URL for persistence.
   * Can be assigned from the RPG music library or uploaded manually.
   */
  backgroundMusic?: string;
...
/**
 * A single entry in the voiceover timeline for a shot.
 * Multiple characters can speak in sequence within a single shot —
 * entries are played in array order when the shot is rendered.
 */
export interface VoiceoverEntry {
  /** Unique identifier for this timeline entry */
  id: string;
  /** Display name of the speaking character (freeform, not required to match an entity) */
  characterName: string;
  /** Optional entity ID if the character maps to a world entity */
  entityId?: string;
  /** The spoken text / dialogue for this entry */
  text: string;
  /** Audio for this entry — base64 data URL or blob URL. Generated via TTS or uploaded manually. */
  audioUrl?: string;
}
 ...
 *
 * Each shot defines ONE camera perspective (angle, framing, movement).
 * Shots chain horizontally (right→left handles) to form a storyboard sequence.
 * The last shot of one scene can connect to the first shot of the next scene.
 *
 * EXAMPLE USAGE:
...
export interface ShotNodeData {
  ...
  /**
   * What happens — a description of the action and events occurring during this shot.
   * Answer the question: "What is happening in this shot?"
   * Example: "The hero leaps across the rooftop gap as guards give chase."
   */
  whatHappens?: string;

  /**
   * Shot caption — describes exactly what is VISIBLE on screen, including
   * camera type (wide, close-up, medium), camera movement (pan, tilt, dolly,
   * tracking, handheld), lens (shallow DoF, anamorphic, etc.), visible elements,
   * and ongoing actions. This is the storyboard panel caption.
   * Example: "TRACKING SHOT — Low angle, camera follows at knee height.
   *           Hero silhouetted against orange sky, guards visible behind."
   */
  shotCaption?: string;

  /**
   * @deprecated Use whatHappens + shotCaption instead.
   * Kept for backwards compatibility with projects that set this field before the split.
   */
  description?: string;

  /** Optional mood/concept image for this shot (storyboard frame) */
  image?: string;
  ...
  /**
   * When true, the music from the previous shot continues playing into this shot
   * without restarting. When false (default), music starts fresh for this shot.
   * If no backgroundMusic is set and this is true, current music just keeps playing.
   */
  musicContinueFromPrevious?: boolean;

  /**
   * Ordered voiceover timeline — a sequence of spoken lines from different
   * characters played in order during this shot. Entries are played sequentially
   * from index 0 upward.
   *
   * This replaces the single voiceoverAudio field for shot nodes.
   * Use this to build multi-character dialogue within a single camera shot.
   */
  voiceoverTimeline?: VoiceoverEntry[];

  /**
   * @deprecated Single-track voiceover. Kept for backwards compat.
   * New shots should use voiceoverTimeline instead.
   */
  voiceoverAudio?: string;
  /**
   * Dictionary of entity state changes during this segment.
 ...
 *
 * Used in the entityStatePatches field on co-writing nodes to describe
 * structured changes to entity profiles that occur during a story segment.
 *
 * Common operations:
 *   { "op": "add",     "path": "/hairColor", "value": "silver" }
 *   { "op": "replace", "path": "/health",    "value": "wounded — deep slash on left arm" }
 *   { "op": "remove",  "path": "/inventory/poisonedDagger" }
 *
 * path uses JSON Pointer syntax (RFC 6901): "/" + field name.
 * Nested fields: "/relationships/Alice" → obj.relationships.Alice
 */
export interface JsonPatchOperation {
  /** The operation to perform */
  op: 'add' | 'remove' | 'replace' | 'copy' | 'move' | 'test';
  /** JSON Pointer (RFC 6901) to the target location, e.g. "/health" or "/status/mood" */
  path: string;
  /** Value to set (required for add, replace, test) */
  value?: unknown;
  /** Source JSON Pointer (required for copy, move) */
  from?: string;
}
